...
const DEFAULT_PARSER_NOTE = "브라우저 파서가 잘못된 HTML을 자동 보정하며, script 태그와 on* 이벤트 속성은 제거합니다.";

const TEST_CASES = Object.freeze({
  1: {
    label: "텍스트 변경",
    html: SAMPLE_HTML
      .replace("Virtual DOM Demo", "Virtual DOM Demo Text Update")
      .replace("Change this text node", "Change only this text with TEST 1")
  },
  2: {
    label: "속성 변경",
    html: SAMPLE_HTML
      .replace('class="demo-section featured"', 'class="demo-section featured contrast"')
      .replace('data-version="1"', 'data-version="2"')
      .replace('data-count="3"', 'data-count="3" data-state="props-updated"')
      .replace('data-tone="warm"', 'data-tone="cool"')
      .replace(
        '<button type="button" class="secondary-button" disabled>Disabled sample</button>',
        '<button type="button" class="secondary-button">Enabled sample</button>'
      )
  },
  3: {
    label: "노드 추가",
    html: SAMPLE_HTML
      .replace("</ul>", '    <li class="feature-item added-item">Add one more node from TEST 3</li>\n  </ul>')
      .replace("</div>", '    <button type="button" class="secondary-button">Extra action</button>\n  </div>')
  },
  4: {
    label: "노드 삭제",
    html: SAMPLE_HTML
      .replace('    <li class="feature-item">Change attributes on this list</li>\n', "")
      .replace('    <a href="https://example.com/docs" class="inline-link" data-track="docs">Read docs</a>\n', "")
  },
  5: {
    label: "태그 교체",
    html: SAMPLE_HTML
      .replace('<p class="intro">', '<article class="intro emphasis-block">')
      .replace("</p>", "</article>")
  }
});
...
function cacheElements() {
  state.elements = {
    ...
    domEditor: document.getElementById("dom-editor"),
    ...
    testButtons: Array.from(document.querySelectorAll("[data-test-id]"))
  };
}
...
function bindUIEvents() {
  ...
  state.elements.htmlEditor.addEventListener("input", handleEditorInput);

  state.elements.testButtons.forEach((button) => {
    button.addEventListener("click", handleLoadTestCase);
  });
}

function handleEditorInput() {
  syncPendingState(state.elements.htmlEditor.value);
}

function handleLoadTestCase(event) {
  const testId = event.currentTarget.dataset.testId;
  const nextCase = TEST_CASES[testId];

  if (!nextCase) {
    return;
  }

  state.elements.htmlEditor.value = nextCase.html;
  syncPendingState(nextCase.html);
  setPatchSummary(`TEST ${testId} (${nextCase.label}) 을 편집 영역에 불러왔습니다. PATCH를 누르면 적용됩니다.`);
}

function buildParsedState(htmlString) {
  const nextDOM = parseHTMLToDOM(htmlString);
  const vNode = domToVirtualDOM(nextDOM);
  const normalizedHTML = virtualDOMToHTML(vNode);
...
function syncPendingState(rawHTML) {
  const nextState = buildParsedState(rawHTML);

  updateParserNote(rawHTML, nextState.normalizedHTML);
  renderPreviewTree(nextState.vNode);
  renderVDOMPreview(nextState.vNode);
...
function applyPatches(rootElement, patches) {
  ...
  const removePatches = patches.filter((patch) => patch.type === "REMOVE").sort(compareRemovePatches);
  const addPatches = patches.filter((patch) => patch.type === "ADD").sort(compareAddPatches);
...
function handlePatch() {
  ...
  const { vNode: nextVNode, normalizedHTML } = buildParsedState(rawHTML);
...
function updateHistoryUI() {
  ...
  state.elements.historyStatus.textContent = `${state.historyIndex} / ${maxIndex}`;
...
function renderDiffLog(patches) {
  ...
  if (!patches.length) {
    container.innerHTML = `
      <div class="empty-log">
        <strong>변경 없음</strong>
        <span>현재 상태와 비교했을 때 새로 적용할 PATCH가 없습니다.</span>
      </div>
    `;
    return;
  }
...
function renderVDOMPreview(vNode) {
  state.elements.domEditor.value = JSON.stringify(vNode, null, 2);
}
  ...
  if (!state.mutationLogs.length) {
    container.innerHTML = `
      <div class="empty-log">
        ...
        <span>PATCH, UNDO, REDO를 실행하면 실제 DOM 변경 로그가 여기에 표시됩니다.</span>
      </div>
    `;
    return;
  }
...
function renderTreeIntoCanvas(canvas, vNode) {
  canvas.innerHTML = "";
...
function renderActualTree(vNode) {
  renderTreeIntoCanvas(state.elements.actualCanvas, vNode);
}

function renderPreviewTree(vNode) {
  renderTreeIntoCanvas(state.elements.previewCanvas, vNode);
}
...
function summarizePatchRun(patches, trimmedFutureCount) {
  ...
  if (trimmedFutureCount > 0) {
    return `${patches.length}개 PATCH 적용 (${countMessage}), future history ${trimmedFutureCount}개를 정리하고 새 상태를 저장했습니다.`;
  }
...
function describePatch(patch) {
  if (patch.type === "TEXT") {
    return `텍스트를 "${truncateText(patch.oldText || "", 42)}" 에서 "${truncateText(patch.text, 42)}" 로 변경합니다.`;
  }
  ...
  if (patch.type === "PROPS") {
    ...
    if (setKeys.length) {
      parts.push(`설정/변경: ${setKeys.map((key) => `${key}=${JSON.stringify(patch.set[key])}`).join(", ")}`);
    }

    if (removeKeys.length) {
      parts.push(`제거: ${removeKeys.join(", ")}`);
    }
  ...
  if (patch.type === "REMOVE") {
    return `${patch.index}번 위치의 자식 노드를 제거합니다. 제거 대상: ${formatVNodeSnippet(patch.node)}`;
  }

  if (patch.type === "REPLACE") {
    return `${describeVNode(patch.oldNode)} 를 ${describeVNode(patch.node)} 로 교체합니다. 이전: ${formatVNodeSnippet(patch.oldNode)} | 이후: ${formatVNodeSnippet(patch.node)}`;
  }

  return "알 수 없는 PATCH입니다.";
}
